...
tf.set_random_seed(777)  # 시드가 없으면 (1, 0, 2 ...), 있으면 (2, 1 0.1...)
...
# Our hypothesis XW+b
hypothesis = x_train * W + b  # H(x) = W*x + b

# cost/loss function
cost = tf.reduce_mean(tf.square(hypothesis - y_train))  # 배열에 있는 원소의 계산결과를 더하여 개수로 나누어줌 --> cost(W,b)
